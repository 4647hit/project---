...
namespace RPC
{
    namespace Client
    {
        ...
        public:
            ...
            {
                return CommonTopic(TopicOptype::TOPIC_PUBLISH, conn, name, msg);
            }
        ...
        private:
            ...
            bool CommonTopic(TopicOptype type, const BaseConnection::ptr &conn, const std::string &name, const std::string &msg = "")
            {
                ...
                bool ret = _request->send(conn,req_message, rsp_msg);
                if (!ret)
                {
                    ELOG("发送同步Topic请求失败");
                    return false;
                ...
                // DLOG("开始等待响应");
                //  3.等待响应
                auto rpc_rsp_msg = std::dynamic_pointer_cast<TopicResponse>(rsp_msg);
                if (!rpc_rsp_msg)
                {
                    ELOG("向下转换失败");
                    return false;
